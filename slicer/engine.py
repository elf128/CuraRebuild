# slicer/engine.py
#
# Orchestrates a complete slice operation for one BuildVolume:
#
#   1. Collect assigned FreeCAD bodies
#   2. Apply world → printer transform to each shape
#   3. Export transformed shapes as STL
#   4. Write CuraEngine definition files (machine.def.json + profile.def.json)
#   5. Invoke CuraEngine as a subprocess
#   6. Return the path to the produced G-code file
#
# The caller is responsible for:
#   - Providing a live SettingsStack (resolved from the registry)
#   - Calling transform_gcode_lines() afterwards if they want world-space
